    ...
    <div className="min-h-screen bg-gradient-to-br from-green-900 via-green-800 to-green-900 relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-10">
       ...
       <div className="relative z-10 p-6">
         ...
         <div className="mb-8">
           <div className="flex justify-between items-start mb-4">
             ...
             <div className="text-right">
               ...
               <div className="text-white text-sm mb-1">{t('lending.welcome')}</div>
                  ...
                  <div key={circle.id} className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm mb-4">
                        ...
                        <div className="font-semibold text-lg">{circle.interestRate}% p.a.</div>
                            ...
                            <div>
                        <span className="text-gray-600">{t('lending.loanRange')}:</span>
                        <div className="font-semibold">{circle.loanRange}</div>
                            </div>
                      <div>
                        <span className="text-gray-600">{t('lending.completedLoans')}:</span>
                        <div className="font-semibold">{circle.completedLoans} loans</div>
                      </div>
                      <div>
                        <span className="text-gray-600">{t('lending.trustScore')}:</span>
                    ...
                    <div className="mb-4">
                      <span className="text-sm text-gray-600">{t('lending.memberContribution')}:</span>
                      <div className="font-semibold text-lg">₹{circle.memberContribution.toLocaleString()}</div>
                    </div>

                    <div className="mb-4 p-3 bg-green-50 rounded-lg">
                      <div className="text-sm text-gray-600">{t('lending.admin')}:</div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-lg p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-gray-800">{t('lending.myLoans')}</h2>
                  ...
                  <div key={loan.id} className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
                    ...
                    {loan.status === 'approved' && loan.dueDate && loan.monthlyEMI && (
                      <div className="mt-4 bg-gray-50 rounded-lg p-4 flex justify-between">
                        <span className="text-sm text-gray-600">{t('lending.dueDate')}: {loan.dueDate}</span>
                        <span className="text-sm text-gray-600">{t('lending.monthlyEMI')}: ₹{loan.monthlyEMI.toLocaleString()}</span>
                      </div>
                    )}
                    
                    {loan.status === 'pending' && loan.appliedDate && (
                      <div className="mt-4 text-sm text-gray-600">
                          {t('lending.appliedOn')} {loan.appliedDate}
                        </div>
                      )}
                      
                    {loan.status === 'repaid' && loan.repaidDate && (
                      <div className="mt-4 text-sm text-gray-600">
                          {t('lending.repaidOn')} {loan.repaidDate}
                        </div>
                      )}
                    </div>
                ))}
        ...
        {currentPage === 'admin' && (
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-lg p-6">
              <h2 className="text-2xl font-bold text-gray-800 mb-6">{t('lending.adminPanel')} - {t('lending.pendingLoanRequests')}</h2>
              <div className="space-y-4">
                  ...
                  <div key={request.id} className="border border-gray-200 rounded-lg p-6 bg-yellow-50">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        ...
                        <div className="text-lg text-gray-600">{t('lending.from')}: {request.userName}</div>
                        <div className="text-sm text-gray-600">Circle: {circles.find(c => c.id === request.circleId)?.name}</div>
                        <div className="text-sm text-gray-600">{t('lending.purpose')}: {request.purpose}</div>
                        <div className="text-sm text-gray-600">{t('lending.duration')}: {request.repaymentDuration}</div>
                        <div className="text-sm text-gray-500">{t('lending.requestedOn')}: {request.requestDate}</div>
                      </div>
                ...
                {loanRequests.filter(req => req.status === 'pending').length === 0 && (
                  <div className="text-center text-gray-500 py-12">
                    ...
                    <div className="text-xl">{t('lending.noPendingRequests')}</div>
                    <div className="text-sm">{t('lending.checkBackLater')}</div>
                </div>
                )}
              </div>
            </div>
      ...
      {showLoanModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-gray-800">{t('lending.requestLoan')}</h2>
              <button
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleSubmitLoan(); }} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{t('lending.selectCircle')}</label>
                <select
                  ...
                  required
                >
                  <option value="">{t('lending.chooseCircle')}</option>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{t('lending.loanAmount')}</label>
                <input
                  type="number"
                  value={loanAmount}
                  onChange={(e) => setLoanAmount(e.target.value)}
                  placeholder={t('lending.enterAmount')}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-green-500 focus:border-green-500"
                  required
                />
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{t('lending.purpose')}</label>
                <textarea
                  value={purpose}
                  onChange={(e) => setPurpose(e.target.value)}
                  placeholder={t('lending.describePurpose')}
                  rows={3}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{t('lending.duration')}</label>
                <select
                  ...
                  required
                >
                  <option value="">{t('lending.selectDuration')}</option>
      ...
      {showProfileModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-gray-800">{t('lending.editProfile')}</h2>
              <button
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleUpdateUserName(); }} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{t('lending.yourName')}</label>
                <input
                  type="text"
                  value={userNameInput}
                  onChange={(e) => setUserNameInput(e.target.value)}
                  placeholder={t('lending.enterYourName')}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-green-500 focus:border-green-500"
                  required
                />
              </div>
